...
- Create: `tests/integration/test_indeterminate_resolution.py`
...
- Create: `tests/unit/application/test_runtime_resolution.py`
- Modify: `tests/integration/test_indeterminate_resolution.py`
- Create: `tests/integration/test_production_wiring.py`
...
- [ ] **Step 1: Run spec-compliance review** against SPEC 5.3, 5.4, 5.8, 6.1, 10.2, 10.4, and the R4.1 plan. Fix every critical/high finding in a separate correction commit.
...
FAIL_RUN and CANCEL_RUN are set-bound atomic store operations, not member shortcuts. A runtime observation coordinator re-observes every member outside the database transaction and rejects the selection before store submission unless every member is proven abandonable. The state store receives the complete typed observation tuple and performs only durable CAS validation. On success it closes the complete set in one expected-sequence transaction, applies objective target completion before persisted cancel/pause before class-specific successor, and preserves the unresolved set on denial. Member resolution updates only its owner and leaves dispatch closed until the last member closes.
...
The reviewed base for R4.1-01 is f601498, the current document correction commit. Before creating each later worktree, the docs-only ledger commit records one exact Final-Reviewed-Base-SHA for the previous task. The worktree command is git worktree add .worktrees/m1-r4-N branch-name Final-Reviewed-Base-SHA; an unexpanded phrase such as reviewed task commit is not an acceptable base. Each task has one corresponding PR record with target branch main and title: PR-R4.1-01 / feat(recovery): classify intent recovery by action class; PR-R4.1-02 / feat(recovery): atomically resolve indeterminate members; PR-R4.1-03 / feat(runtime): execute exact indeterminate resolutions; PR-R4.1-04 / fix(runtime): recover asymmetric target cleanup safely. Remote PR creation and push remain pending explicit human authorization and are never represented as completed before observed.
...
Each RecoveryObservation variant has a discriminant kind, intent_id, action_class, recovery_generation, source_payload_digest, state, and canonical observation_digest. MODEL adds request_digest, provider_response_id, returned_model_id, schema_digest, usage_json, reservation_charge, and normalized_completion_digest. READ_SEARCH adds snapshot_digest, scope_digest, ordering_digest, bounded_result_digest, and a bounded_result_json internal payload only for EXACT_SNAPSHOT; the payload is canonical, size-bounded, sanitized, and its digest is included in observation_digest, while bounded result bytes are forbidden for STALE. PATCH adds expected_pre_tree_digest and observed_post_tree_digest. CHECK adds check_id, argv_digest, snapshot_digest, and receipt_digest. REF adds repository_id, repository_instance_digest, ref_name, registration_digest, target_safety_digest, old_oid, prepared_oid, and current_oid. RESERVATION adds registration_identity, admin_binding_digest, path_identity, gitfile_digest, and state. Pydantic extra-forbid validation, sorted canonical JSON, and the listed fields form the digest input; no variant accepts credential, nonce, raw provider transcript, or arbitrary human text.

RecoveryDecision is closed: COMPLETED with an EffectResult for authoritative post-state, RETRY_SAME_INTENT for an authoritative pre-state whose action class is replay-safe, ABANDONED with a class-specific successor, STALE, CONFLICT, or INDETERMINATE. COMPLETED requires result_digest, bounded_result_json, settled_sequence, and the matching applicable revisions; RETRY_SAME_INTENT requires action_class, exact prestate digest, idempotency key, and replay-safe proof; ABANDONED requires action_class, successor code, and no EffectResult; STALE/CONFLICT/INDETERMINATE require reason code and no external-result payload. ABANDONED is never written as an EffectResult outcome; the state transaction writes the typed owner successor and a sanitized Audit result class. No plan step uses an undefined RETRY or ABANDON result.

The exact matrix selectors are mandatory. Task 1 runs tests/unit/domain/test_recovery.py::test_model_unknown_provider_is_not_retryable, tests/unit/domain/test_recovery.py::test_read_search_replays_only_same_snapshot, tests/unit/domain/test_recovery.py::test_patch_replays_only_exact_prestate, tests/unit/domain/test_recovery.py::test_check_reuses_exact_receipt, tests/unit/domain/test_recovery.py::test_private_ref_and_target_cas_require_all_safety_fields, tests/unit/domain/test_recovery.py::test_reservation_observation_classifies_asymmetric_states, tests/unit/domain/test_indeterminate.py::test_resolution_selection_requires_exact_member_or_set_shape, and tests/unit/domain/test_indeterminate.py::test_unresolved_set_digest_changes_after_member_close; expected red is 8 failed and green is 8 passed. Task 2 runs tests/integration/test_indeterminate_resolution.py::test_resolution_command_issues_subject_bound_permit, ::test_resolution_command_rejects_stale_set_without_sequence_change, ::test_member_resolution_reopens_only_after_last_member, ::test_set_cancel_is_denied_when_any_member_is_not_abandonable, ::test_set_fail_closes_all_abandonable_members_atomically, ::test_resolution_permit_replay_cannot_change_selection, and ::test_sqlite_reopen_preserves_resolution_subject_and_set_digest; expected red is 7 failed and green is 7 passed. Task 3 runs tests/unit/application/test_runtime_resolution.py::test_resolution_runtime_requires_consumed_subject_bound_permit, ::test_reconcile_observed_uses_internal_observation_not_command_data, ::test_retry_same_intent_requires_exact_prestate, ::test_abandon_returns_class_specific_successor, ::test_runtime_owner_and_active_time_close_on_resolution_stop, and tests/integration/test_production_wiring.py::test_production_bundle_has_no_deferred_resolution_driver; expected red is 6 failed and green is 6 passed. Task 4 runs tests/unit/adapters/repository/test_target_reservation_cleanup.py::test_exact_path_only_cleanup_removes_only_gitfile_and_empty_directory, ::test_exact_admin_only_cleanup_removes_only_bound_admin_entry, ::test_mixed_cleanup_conflict_deletes_nothing, ::test_altered_admin_digest_deletes_nothing, ::test_unobservable_cleanup_preserves_terminal_run, and tests/integration/test_composed_runtime_lifecycle.py::test_cleanup_settlement_requires_exact_absence_after_reopen; expected red is 6 failed and green is 6 passed. Full-file or keyword-only selectors are not evidence for these tasks.
...
The final executable Task 2 command, superseding the earlier seven-node command, is uv run --python 3.12 pytest tests/integration/test_indeterminate_resolution.py::test_resolution_command_issues_subject_bound_permit tests/integration/test_indeterminate_resolution.py::test_resolution_command_rejects_stale_set_without_sequence_change tests/integration/test_indeterminate_resolution.py::test_member_resolution_reopens_only_after_last_member tests/integration/test_indeterminate_resolution.py::test_set_cancel_is_denied_when_any_member_is_not_abandonable tests/integration/test_indeterminate_resolution.py::test_set_fail_closes_all_abandonable_members_atomically tests/integration/test_indeterminate_resolution.py::test_resolution_permit_replay_cannot_change_selection tests/integration/test_indeterminate_resolution.py::test_sqlite_reopen_preserves_resolution_subject_and_set_digest tests/integration/test_indeterminate_resolution.py::test_set_bound_cancel_precedence_over_class_successor tests/integration/test_indeterminate_resolution.py::test_set_bound_target_completion_precedence_over_cancel tests/integration/test_indeterminate_resolution.py::test_set_bound_pause_precedence_after_last_member -q.

The final executable Task 3 command, superseding the earlier six-node command, is uv run --python 3.12 pytest tests/unit/application/test_runtime_resolution.py::test_resolution_runtime_requires_consumed_subject_bound_permit tests/unit/application/test_runtime_resolution.py::test_reconcile_observed_uses_internal_observation_not_command_data tests/unit/application/test_runtime_resolution.py::test_retry_same_intent_requires_exact_prestate tests/unit/application/test_runtime_resolution.py::test_abandon_returns_class_specific_successor tests/unit/application/test_runtime_resolution.py::test_runtime_owner_and_active_time_close_on_resolution_stop tests/integration/test_production_wiring.py::test_production_bundle_has_no_deferred_resolution_driver tests/unit/application/test_runtime_resolution.py::test_runtime_budget_interrupt_barrier_is_atomic tests/unit/application/test_runtime_resolution.py::test_resolution_audit_and_query_projection_are_redacted -q.

Task 1's 18 nodes are all in tests/unit/domain/test_recovery.py except resolution_selection_requires_exact_member_or_set_shape in tests/unit/domain/test_indeterminate.py. Task 2's seven original plus three precedence nodes are all in tests/integration/test_indeterminate_resolution.py. Task 3's first six nodes are in tests/unit/application/test_runtime_resolution.py except test_production_bundle_has_no_deferred_resolution_driver in tests/integration/test_production_wiring.py; the two added nodes are in tests/unit/application/test_runtime_resolution.py. Task 4's first five nodes are in tests/unit/adapters/repository/test_target_reservation_cleanup.py and test_cleanup_settlement_requires_exact_absence_after_reopen is in tests/integration/test_composed_runtime_lifecycle.py. The Task 1 shell command is uv run --python 3.12 pytest tests/unit/domain/test_recovery.py::model_completed_normalized_response tests/unit/domain/test_recovery.py::model_authoritative_lookup_exact_response tests/unit/domain/test_recovery.py::model_returned_model_mismatch_charges_full_reservation tests/unit/domain/test_recovery.py::model_unavailable_is_indeterminate tests/unit/domain/test_recovery.py::read_search_same_snapshot_returns_bounded_payload tests/unit/domain/test_recovery.py::read_search_changed_scope_is_stale_without_content tests/unit/domain/test_recovery.py::patch_exact_post_is_completed tests/unit/domain/test_recovery.py::patch_exact_pre_is_retryable tests/unit/domain/test_recovery.py::patch_third_state_is_conflict tests/unit/domain/test_recovery.py::check_exact_receipt_collapses_duplicate tests/unit/domain/test_recovery.py::check_exact_pre_reruns_same_argv tests/unit/domain/test_recovery.py::private_ref_requires_identity_and_registration_digest tests/unit/domain/test_recovery.py::target_cas_prepared_is_completed tests/unit/domain/test_recovery.py::target_cas_exact_old_is_retryable tests/unit/domain/test_recovery.py::target_cas_third_state_is_conflict tests/unit/domain/test_recovery.py::target_cas_target_unsafe_is_stale tests/unit/domain/test_recovery.py::reservation_creation_matrix_covers_absent_unlocked_locked_mixed_unobservable tests/unit/domain/test_indeterminate.py::resolution_selection_requires_exact_member_or_set_shape -q.

The executable Task 2 selector is: uv run --python 3.12 pytest tests/integration/test_indeterminate_resolution.py::test_resolution_command_issues_subject_bound_permit tests/integration/test_indeterminate_resolution.py::test_resolution_command_rejects_stale_set_without_sequence_change tests/integration/test_indeterminate_resolution.py::test_member_resolution_reopens_only_after_last_member tests/integration/test_indeterminate_resolution.py::test_set_cancel_is_denied_when_any_member_is_not_abandonable tests/integration/test_indeterminate_resolution.py::test_set_fail_closes_all_abandonable_members_atomically tests/integration/test_indeterminate_resolution.py::test_resolution_permit_replay_cannot_change_selection tests/integration/test_indeterminate_resolution.py::test_sqlite_reopen_preserves_resolution_subject_and_set_digest -q. The executable Task 3 selector is: uv run --python 3.12 pytest tests/unit/application/test_runtime_resolution.py::test_resolution_runtime_requires_consumed_subject_bound_permit tests/unit/application/test_runtime_resolution.py::test_reconcile_observed_uses_internal_observation_not_command_data tests/unit/application/test_runtime_resolution.py::test_retry_same_intent_requires_exact_prestate tests/unit/application/test_runtime_resolution.py::test_abandon_returns_class_specific_successor tests/unit/application/test_runtime_resolution.py::test_runtime_owner_and_active_time_close_on_resolution_stop tests/integration/test_production_wiring.py::test_production_bundle_has_no_deferred_resolution_driver -q. The executable Task 4 selector is: uv run --python 3.12 pytest tests/unit/adapters/repository/test_target_reservation_cleanup.py::test_exact_path_only_cleanup_removes_only_gitfile_and_empty_directory tests/unit/adapters/repository/test_target_reservation_cleanup.py::test_exact_admin_only_cleanup_removes_only_bound_admin_entry tests/unit/adapters/repository/test_target_reservation_cleanup.py::test_mixed_cleanup_conflict_deletes_nothing tests/unit/adapters/repository/test_target_reservation_cleanup.py::test_altered_admin_digest_deletes_nothing tests/unit/adapters/repository/test_unobservable_cleanup_preserves_terminal_run tests/integration/test_composed_runtime_lifecycle.py::test_cleanup_settlement_requires_exact_absence_after_reopen -q.

The final executable Task 1 selector uses the test_ names declared by the test file: uv run --python 3.12 pytest tests/unit/domain/test_recovery.py::test_model_completed_normalized_response tests/unit/domain/test_recovery.py::test_model_authoritative_lookup_exact_response tests/unit/domain/test_recovery.py::test_model_returned_model_mismatch_charges_full_reservation tests/unit/domain/test_recovery.py::test_model_unavailable_is_indeterminate tests/unit/domain/test_recovery.py::test_read_search_same_snapshot_returns_bounded_payload tests/unit/domain/test_recovery.py::test_read_search_changed_scope_is_stale_without_content tests/unit/domain/test_recovery.py::test_patch_exact_post_is_completed tests/unit/domain/test_recovery.py::test_patch_exact_pre_is_retryable tests/unit/domain/test_recovery.py::test_patch_third_state_is_conflict tests/unit/domain/test_recovery.py::test_check_exact_receipt_collapses_duplicate tests/unit/domain/test_recovery.py::test_check_exact_pre_reruns_same_argv tests/unit/domain/test_recovery.py::test_private_ref_requires_identity_and_registration_digest tests/unit/domain/test_recovery.py::test_target_cas_prepared_is_completed tests/unit/domain/test_recovery.py::test_target_cas_exact_old_is_retryable tests/unit/domain/test_recovery.py::test_target_cas_third_state_is_conflict tests/unit/domain/test_recovery.py::test_target_cas_target_unsafe_is_stale tests/unit/domain/test_recovery.py::test_reservation_creation_matrix_covers_absent_unlocked_locked_mixed_unobservable tests/unit/domain/test_indeterminate.py::test_resolution_selection_requires_exact_member_or_set_shape -q.

The preceding eight/seven/six node summaries are superseded by this complete matrix. Task 1's executable selector contains 18 nodes and expects red 18 failed then green 18 passed: model_completed_normalized_response, model_authoritative_lookup_exact_response, model_returned_model_mismatch_charges_full_reservation, model_unavailable_is_indeterminate, read_search_same_snapshot_returns_bounded_payload, read_search_changed_scope_is_stale_without_content, patch_exact_post_is_completed, patch_exact_pre_is_retryable, patch_third_state_is_conflict, check_exact_receipt_collapses_duplicate, check_exact_pre_reruns_same_argv, private_ref_requires_identity_and_registration_digest, target_cas_prepared_is_completed, target_cas_exact_old_is_retryable, target_cas_third_state_is_conflict, target_cas_target_unsafe_is_stale, reservation_creation_matrix_covers_absent_unlocked_locked_mixed_unobservable, and resolution_selection_requires_exact_member_or_set_shape. Task 2 adds set_bound_cancel_precedence_over_class_successor, set_bound_target_completion_precedence_over_cancel, and set_bound_pause_precedence_after_last_member; its executable selector contains 10 nodes and expects red 10 failed then green 10 passed. Task 3 adds runtime_budget_interrupt_barrier_is_atomic and resolution_audit_and_query_projection_are_redacted; its executable selector contains 8 nodes and expects red 8 failed then green 8 passed. Each node is created in the File Map test file before its red command; no summary count substitutes for a node.
