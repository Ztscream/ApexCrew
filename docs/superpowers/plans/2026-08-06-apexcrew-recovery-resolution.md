...
Each RecoveryObservation variant has a discriminant kind, intent_id, action_class, recovery_generation, source_payload_digest, state, and canonical observation_digest. MODEL adds request_digest, provider_response_id, returned_model_id, schema_digest, usage_json, reservation_charge, and normalized_completion_digest. READ_SEARCH adds snapshot_digest, scope_digest, ordering_digest, and bounded_result_digest, with bounded result bytes forbidden for STALE. PATCH adds expected_pre_tree_digest and observed_post_tree_digest. CHECK adds check_id, argv_digest, snapshot_digest, and receipt_digest. REF adds repository_id, repository_instance_digest, ref_name, registration_digest, target_safety_digest, old_oid, prepared_oid, and current_oid. RESERVATION adds registration_identity, admin_binding_digest, path_identity, gitfile_digest, and state. Pydantic extra-forbid validation, sorted canonical JSON, and the listed fields form the digest input; no variant accepts credential, nonce, raw provider transcript, or arbitrary human text.
