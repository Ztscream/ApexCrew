...
`SPEC.md` remains byte-frozen. The formal implementation base is `9e7648f` on `codex/m2-m4-final-production`, containing the reviewed R4.1 recovery/cleanup chain and verified M2-M4 provider, Docker, retention, CI, WebUI, and build work. The primary checkout at `3676526` has an uncommitted diagnostic/demo draft; it is not an implementation base and no unreviewed source/test/fixture/CI change may be copied from it. After a zero-blocker document review, create a docs-only reviewed-plan commit descended from `9e7648f`, containing only `PLAN.md`, this companion plan, and the review/owner-GO record in `AGENT_LOG.md`. The first implementation worktree must be created from that exact commit and must prove that ancestry before any source/test/fixture/CI change.
...
- [ ] Create a docs-only reviewed-plan commit descended from `9e7648f`, prove its changed paths are only `PLAN.md`, this companion plan, and the review/GO record in `AGENT_LOG.md`, then create `.worktrees/m1-r4-3-demo-loop` from that exact commit and verify `git status --short --branch` is empty.
