...
class SqliteStateStore:
    ...
    def settle_model_attempt(
        ...
        def mutate(connection: sqlite3.Connection) -> None:
            ...
            changed = connection.execute(
                ...
                "response_requested_model_id = ?, returned_model_id = ?, result_digest = ?, "
                "charged_json = ?, "
            ...
            if settled.kind is ProviderAttemptKind.COMPLETED:
                ...
                if dispatch.outcome == "COMPLETED":
                    if (
                        ...
                        or dispatch.response_requested_model_id
                    ...
                    committed = connection.execute(
                        ...
                        "response_requested_model_id = ?, returned_model_id = ?, "
    ...
    def model_attempts(
        ...
        rows = self._connection.execute(
            ...
            "charged_json, result_json, response_requested_model_id, reported_usage_json, "
            "backoff_seconds FROM model_attempts "
        ...
        for row in rows:
            ...
            if dispatch.response_requested_model_id != row[11]:
                raise StateConflict("MODEL_RESPONSE_REQUESTED_ID_STORAGE_BINDING_MISMATCH")
            reported_usage = (
                None
                if row[12] is None
                else ModelUsage(
                    input_tokens=int(json.loads(row[12])["input_tokens"]),
                    output_tokens=int(json.loads(row[12])["output_tokens"]),
                    cost_usd=Decimal(str(json.loads(row[12])["cost_usd"])),
                )
            )
            attempts.append(
                SettledModelAttempt(
    ...
    def committed_model_turn(
        ...
        row = self._connection.execute(
            ...
            "recovery_binding_json, response_requested_model_id, returned_model_id, "
        ...
        if row is None or row[12] not in {
            "COMPLETION_COMMITTED",
            "DOWNSTREAM_INTENT_RECORDED",
        }:
            return None
        if any(row[index] is None for index in (1, 5, 7, 8, 9, 10, 11)):
        ...
        dispatch = model_dispatch_result_from_json(row[10])
        payload = json.loads(row[9])
        if row[6] is None:
            if response_requested_model_id_present:
                raise StateConflict(
        ...
        if (
            ...
            or dispatch.returned_model_id != row[7]
            or dispatch.normalized_payload_digest != row[8]
        ...
        return CommittedModelTurn(
            ...
            returned_model_id=row[7],
            normalized_output_digest=row[8],
            normalized_payload=payload,
            dispatch_result=dispatch,
            committed_sequence=AuditSequence(row[11]),
            state=row[12],
            downstream_intent_id=(None if row[13] is None else IntentId(row[13])),
            downstream_sequence=(None if row[14] is None else AuditSequence(row[14])),
        )
